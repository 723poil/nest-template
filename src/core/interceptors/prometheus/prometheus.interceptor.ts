...
export class PrometheusInterceptor implements NestInterceptor, OnModuleInit {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    const originUrl: string = context.switchToHttp().getRequest<{ url: string; method: string }>().url.toString();
    const method: string = context.switchToHttp().getRequest<{ url: string; method: string }>().method.toString();
    const labels = {
    ...
    try {
      ...
      winstonLogger.error("[PROMETHEUS] handler error");
    }
  }
